			...
			<div class="container">
				...
				<div class="row">
                    <div class="d-flex">
                        ...
                        <h3 style="color:#D10024;"> Discription Of Product : <br><br><textarea style="color:rgba(0, 0, 0, 0.5); resize:none; font-size: 18px; text-align:left;padding: 15px;line-height: 24px; height:150px" readonly  cols="23" rows="auto">{{$product->description}}</textarea></h3>
                        </div>
					    <!-- Product details -->
					    <div class="product-details col-xs-4">
                            <h3 style="color:#D10024;">Product Name</h3>
					        <h2  style="font-weight:bold;" class="product-name">{{$product->name}}</h2>
					    	<div>
                                <h3 style="color:#D10024;">Product Price</h3>
					    		<h4 style="color:black; font-size:18px;" class="product-price"> $ {{$product->price}}</h4>
					    	</div>
                            @foreach ($product->productSpecs as $spec)
                            <h3 style="color:#D10024;">{{$spec->categorySpec->name}}</h3>
